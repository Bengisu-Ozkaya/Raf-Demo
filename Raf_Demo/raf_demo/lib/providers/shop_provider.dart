...
import '../models/shop_package.dart'; // YENİ: Paket modeli
...
class ShopProvider with ChangeNotifier {
  ...
  List<Product> _merchantProducts = []; // İşletmecinin kendi ürünleri
  List<ShopPackage> _merchantPackages = []; // YENİ: İşletmecinin paketleri
  List<Product> _masterProducts =
      []; // Master ürünler (işletmeci ekleme ekranı için)
  List<MerchantOrder> _merchantOrders = []; // İşletmecinin siparişleri
  ...
  List<ShopPackage> get merchantPackages => _merchantPackages; // YENİ: Paketler
  List<Product> get masterProducts => _masterProducts;
  List<MerchantOrder> get merchantOrders => _merchantOrders;
  ...
  /// Seçilen paketteki tüm ürünleri topluca ekler ve dükkan listesini tazeler.
  Future<bool> addProductsBatch({
    ...
    notifyListeners();
    try {
      await _apiService.addProductsBatch(products: products);
      _merchantProducts = await _apiService.fetchProductsForMerchant(shopId);
      _isLoading = false;
      notifyListeners();
      return true;
  ...
  // --- PAKET İŞLEMLERİ (PACKAGE STATE & METHODS) ---
  ...
  Future<List<ShopPackage>> fetchPackages(int shopId) async {
    ...
    notifyListeners();
    try {
      _merchantPackages = await _apiService.fetchPackages(shopId);
      _isLoading = false;
      notifyListeners();
      return _merchantPackages;
    } catch (e) {
      _errorMessage = e.toString();
      _isLoading = false;
      notifyListeners();
  ...
  /// İşletmecinin paketlerini getirir.
  ...
  /// Yeni bir paket oluşturur ve dükkan listesini tazeler.
  ...
  /// Bir paketi siler.
  Future<bool> deletePackage(int packageId, int shopId) async {
    try {
      await _apiService.deletePackage(packageId);
      _merchantPackages.removeWhere((p) => p.id == packageId);
      notifyListeners();
      return true;
    } catch (e) {
      _errorMessage = e.toString();
