...
class CartShop {
  ...
  final Map<int, CartItem> items; // Product ID -> CartItem
...
class CartProvider with ChangeNotifier {
  ...
  // Ürünü ilk defa sepete ekler (ProductGridItem'dan çağrılır)
  void addItem(Product product, Shop shop, {int quantity = 1}) {
    // Stok ve miktar kontrolü
    if ((product.stock ?? 0) <= 0 || quantity <= 0) {
      return;
    }

    // Eğer dükkan sepette yoksa oluştur
    if (!_shops.containsKey(shop.id)) {
      _shops[shop.id] =
          CartShop(shopId: shop.id, shopName: shop.name, items: {});
    }

    final cartShop = _shops[shop.id]!;

    // Eğer ürün o dükkanın sepetinde varsa miktarını artır
    if (cartShop.items.containsKey(product.id)) {
      final item = cartShop.items[product.id]!;
      final newQuantity = item.quantity + quantity;

      // Yeni miktar stok limitini aşıyor mu kontrol et
      item.quantity = min(newQuantity, product.stock ?? 0);
    } else {
      // Yoksa yeni bir CartItem olarak ekle. Eklenecek miktar stoktan fazlaysa, stok kadar ekle.
      final quantityToAdd = min(quantity, product.stock ?? 0);
      cartShop.items[product.id] =
          CartItem.fromProduct(product, quantity: quantityToAdd);
    }
